...
-- a function that converts the current latex document to markdown (pandoc):
vim.api.nvim_create_user_command(
  'ConvertLatexToMarkdown', 
  function()
    vim.cmd([[
      execute '!pandoc -s % -o %:r.md --shift-heading-level-by=1'
    ]])
  end,
  {
    desc = 'Convert current latex document to markdown',
    nargs = 0 -- This command takes no arguments
  }
)
-- bind it to <leader>clm
vim.api.nvim_set_keymap('n', '<leader>clm', ':ConvertLatexToMarkdown<CR>', { noremap = true, silent = true })


-- a function that deletes all lines starting with a % (latex comment):
vim.api.nvim_create_user_command(
  'DeleteLatexComments', 
  function()
    vim.cmd([[
  ...
  {
    desc = 'Delete all lines starting with % (latex comment)',
    nargs = 0 -- This command takes no arguments
  }
)
-- bind it to <leader>dlc
vim.api.nvim_set_keymap('n', '<leader>dlc', ':DeleteLatexComments<CR>', { noremap = true, silent = true })

vim.api.nvim_create_user_command(
  'ML', 
  function()
    if vim.fn.filereadable('Makefile') or vim.fn.filereadable('makefile') then
      vim.notify("Running make local in background...", vim.log.levels.INFO)

      vim.fn.jobstart('make local', {
        on_stdout = function(job_id, data, event) end, -- Suppress stdout
        on_stderr = function(job_id, data, event) end, -- Suppress stderr
        detach = true, -- Crucial for running in the background without blocking Neovim
      })
    else
      vim.notify("Error: No Makefile found in current directory.", vim.log.levels.ERROR)
    end
  end,
  {
    desc = 'Run make local silently in background (if Makefile exists)',
    nargs = 0 -- This command takes no arguments
  }
)
vim.api.nvim_set_keymap('n', '<F12>', ':ML<CR>', { noremap = true, silent = true })
