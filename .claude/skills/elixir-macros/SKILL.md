...
Call `mcp__elixir-mcp__get_module_docs` (from the `elixir-mcp` MCP server) with `"macros"` for the macro guide, `"quote-and-unquote"` for quote mechanics, or `"Macro"` for the `Macro` module API.
