...
Call `mcp__elixir-mcp__get_module_docs` (from the `elixir-mcp` MCP server) with `"GenServer"` for the complete callback reference including `terminate/2`, `code_change/3`, and `format_status/1`.
