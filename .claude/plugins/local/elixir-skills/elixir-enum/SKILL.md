...
```
get_module_docs({ module: "Enum" })       # Full Enum API (3000+ lines)
get_module_docs({ module: "enum-cheat" }) # Quick cheatsheet with examples
get_module_docs({ module: "Stream" })     # Lazy counterpart to Enum
get_module_docs({ module: "MapSet" })     # Set operations
get_module_docs({ module: "Keyword" })    # Keyword list operations
```
...
## For search
```
search_docs({ query: "chunk flat_map zip group" })
```
